...
//为什么不用long（32位），因为左移右移只能移16位，否则会简单一些
...
void Int0_Routine() interrupt 0
{
	...
	{
		//P2 = 0;	//测试
		IR_Time = Timer0_GetCounter();		//获取计时时间
		Timer0_SetCounter(0);
		if(IR_Time > (int)((float)(13000)/1.085) && IR_Time < (int)((float)(14000)/1.085))	
		//给上下500的范围,13500左右的冗余，找到start帧，状态转化为接收数据
		{
			//P2 = 0;	//测试
			IR_State = 2;
		}
		else if(IR_Time > (int)((float)(10750)/1.085) && IR_Time < (int)((float)(11750)/1.085))	
		//11250，找到repeat帧说明数据接收完了
		{
			IR_RepeatFlag = 1;
			Timer0_Run(0);		//停止计时
			IR_State = 0;
	...
	else if(IR_State == 2)
	{
		...
		if(IR_pData >= 32)	//判断数据是否接收完
		{
			...
			if((IR_Data[0] == ~IR_Data[1]) && (IR_Data[2] == ~IR_Data[3]))	
				//数据验证，数据形式：Address，~Address，Command，~Command
			{
